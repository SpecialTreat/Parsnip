    ...
    for (NSString *dataType in dataTypes) {
        ...
        if([dataType isEqualToString:@"PhoneNumber"]) {
            ...
            for(NSArray *data in dataTypes[dataType]) {
                ...
                label = [PreenTextData addressBookLabelForPhoneNumber:data[0] defaultLabel:label];
...
{
    return [PreenTextData detectDataTypes:text stripEmpty:NO];
}
...
+ (void)detectDataTypes:(NSString *)text stripEmpty:(BOOL)stripEmpty completion:(void(^)(NSDictionary *dataTypes))completion
{
    [PreenThread background:^{
        NSDictionary *dataTypes = [PreenTextData detectDataTypes:text stripEmpty:stripEmpty];
        if(completion) {
            [PreenThread main:^{
...
{
    return [PreenTextData detectDataTypesCount:text stripEmpty:NO];
}
