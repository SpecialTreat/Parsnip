...
+ (void)initialize
{
    _needsInitialData = ![[NSFileManager defaultManager] fileExistsAtPath:PreenDB.pathForDB];
    [PreenDB queue];
}
...
{
    if(!_queue) {
        _queue = [FMDatabaseQueue databaseQueueWithPath:PreenDB.pathForDB];
    }
    return _queue;
}
...
+ (NSString *)pathForFiles
{
    if(!_pathForFiles) {
        ...
        _pathForFiles = [documentPath stringByAppendingPathComponent:@"PreenFiles"];
...
+ (NSString *)pathForDB
{
    if(!_pathForDB) {
        NSArray *documentPaths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentPath = ([documentPaths count] > 0) ? [documentPaths objectAtIndex:0] : nil;
        _pathForDB = [documentPath stringByAppendingPathComponent:@"PreenDB.db"];
    }
    return _pathForDB;
}
...
+ (void)loadInitialData
{
    ...
    NSString *initialFilesPath = [initialDataPath stringByAppendingPathComponent:@"PreenFiles"];
    ...
    if ([fileManager fileExistsAtPath:initialFilesPath isDirectory:&isDir] && isDir) {
        ...
        for (NSString *fileName in files) {
            ...
            error = nil;
            [fileManager copyItemAtPath:path toPath:[PreenDB.pathForFiles stringByAppendingPathComponent:fileName] error:&error];
            if (error) {
            ...
            for (NSString *fileName in files) {
                if ([fileName hasSuffix:@".json"]) {
                    ...
                    if ([fileManager fileExistsAtPath:path isDirectory:&isDir] && !isDir) {
                        ...
                        [is close];
                        if (values && !error) {
                            PreenNote *note = [[PreenNote alloc] init];
...
+ (void)register:(Class)cls
{
    [_queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        NSString *sql = [PreenDB sqlForCreateTable:cls];
...
{
    [PreenDB query:sql parameters:nil results:block];
}
...
{
    return [PreenDB count:sql parameters:nil];
}

+ (NSInteger)count:(NSString *)sql parameters:(NSDictionary *)parameters
{
    __block NSInteger count = 0;
    [PreenDB query:sql parameters:parameters results:^(FMResultSet *results) {
        if ([results next]) {
...
+ (BOOL)remove:(id)models
{
    ...
    [_queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for(PreenModel *model in modelList) {
        ...
        for (PreenModel *model in deletedModels) {
            if (success) {
                [model deleteFiles];
            } else {
...
{
    NSString *sql = [PreenDB sqlForDelete:model];
    NSString *pk = [model.class primaryKey];
...
+ (BOOL)save:(id)models
{
    ...
    for(PreenModel *model in modelList) {
        ...
        [_queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
            ...
            success = [PreenDB saveInDatabase:db model:model];
            if(!success) {
                *rollback = YES;
                return;
            }
...
{
    NSString *sql = [PreenDB sqlForInsertOrReplace:model];
...
{
    NSArray *models = [PreenDB get:cls parameters:@{[cls primaryKey]: pk}];
    if(models.count) {
...
{
    return [PreenDB get:cls parameters:parameters orderBy:nil asc:YES];
}

+ (NSArray *)get:(Class)cls parameters:(NSDictionary *)parameters orderBy:(NSString *)orderBy
{
    return [PreenDB get:cls parameters:parameters orderBy:orderBy asc:YES limit:0 offset:0];
}

+ (NSArray *)get:(Class)cls parameters:(NSDictionary *)parameters orderBy:(NSString *)orderBy asc:(BOOL)asc
{
    return [PreenDB get:cls parameters:parameters orderBy:orderBy asc:asc limit:0 offset:0];
}
...
+ (NSArray *)get:(Class)cls parameters:(NSDictionary *)parameters orderBy:(NSString *)orderBy asc:(BOOL)asc limit:(NSUInteger)limit offset:(NSUInteger)offset
{
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        models = [PreenDB getInDatabase:db modelClass:cls parameters:parameters orderBy:orderBy asc:asc limit:limit offset:offset];
    }];
    return models;
}
...
+ (NSArray *)getInDatabase:(FMDatabase *)db modelClass:(Class)cls parameters:(NSDictionary *)parameters orderBy:(NSString *)orderBy asc:(BOOL)asc limit:(NSUInteger)limit offset:(NSUInteger)offset
{
    ...
    NSString *sql = [PreenDB sqlForSelect:cls parameters:parameters orderBy:orderBy asc:asc limit:limit offset:offset];
    parameters = [self prepareParameters:parameters];
    ...
    while([rs next]) {
        PreenModel *model = [[cls alloc] init];
