...
//NSString *const CAPTURE_QUALITY = AVCaptureSessionPresetPhoto;
//NSString *const CAPTURE_QUALITY = AVCaptureSessionPreset1280x720;
NSString *const CAPTURE_QUALITY = AVCaptureSessionPresetHigh;


#if TARGET_IPHONE_SIMULATOR


@implementation PreenCameraView
{
    UIImageView *preview;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        toggleFlashButtonAlpha = [PreenUI.theme floatForKey:@"CaptureToggleFlashButton.Alpha"];
        toggleFlashButtonWidth = [PreenUI.theme floatForKey:@"CaptureToggleFlashButton.Width"];
        toggleFlashButtonHeight = [PreenUI.theme floatForKey:@"CaptureToggleFlashButton.Height"];
        toggleFlashButtonMargin = [PreenUI.theme edgeInsetsForKey:@"CaptureToggleFlashButton.Margin"];
        [self initSubviews];
...
- (void)initSubviews
{
    preview = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"PreenCameraView_Simulator.png"]];
    ...
    UIImage *image = [PreenUI.theme imageForKey:@"CaptureToggleFlashButton.Image"];
    ...
    UIImage *selectedImage = [PreenUI.theme imageForKey:@"CaptureToggleFlashButton.SelectedImage"];
...
{
//    imageHandler([UIImage imageNamed:@"PreenCameraView_SimulatorCropped.png"]);
    imageHandler([UIImage imageNamed:@"PreenCameraView_Simulator.png"]);
}
...
- (void)initSubviews
{
    ...
    UIImage *focusBoxLight = [PreenUI.theme imageForKey:@"FocusBoxLight"];
    UIImage *focusBoxDark = [PreenUI.theme imageForKey:@"FocusBoxDark"];
    UIImage *focusBoxGlow = [PreenUI.theme imageForKey:@"FocusBoxGlow"];
 ...
 - (void)onDeviceMotion:(CMDeviceMotion*)motion
{
    ...
    if(fabs(acceleration.x) > accelerationThreshold || 
       ...
       fabs(acceleration.z) > accelerationThreshold) {
        [PreenDevice.motionManager stopDeviceMotionUpdates];
...
{
    if(PreenDevice.motionManager.deviceMotionAvailable) {
        [PreenDevice.motionManager startDeviceMotionUpdatesToQueue:[NSOperationQueue currentQueue]
...
- (void)showFocusBox
{
    ...
    if(PreenDevice.motionManager.deviceMotionAvailable) {
        [PreenDevice.motionManager startDeviceMotionUpdates];
    }
}
