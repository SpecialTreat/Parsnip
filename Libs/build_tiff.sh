...
setenv_all()
{
    export CFLAGS="$CFLAGS -I$GLOBAL_OUTDIR/include -I$GLOBAL_OUTDIR/include/zlib"
...
do
    ...
    ./configure --host=$host --enable-shared=no --disable-jpeg --with-zlib-lib-dir="$GLOBAL_OUTDIR/lib" --with-zlib-include-dir="$GLOBAL_OUTDIR/include/zlib"
    cp config.log config-$arch.log
    make -j4
    cp -rvf libtiff/.libs/lib*.a $LOCAL_OUTDIR/$arch
done

for lib in `find libtiff/.libs -name "lib*\.a"`; do
    cmd="xcrun -sdk iphoneos lipo "
    for arch in "${archs[@]}";
    do
        lib_arch=`echo $lib | sed "s/libtiff\/\.libs/$ESCAPED_OUTDIR\/$arch/g"`
        cmd+="-arch $arch $lib_arch "
    done
    lib_out=`echo $lib | sed "s/libtiff\/\.libs/$ESCAPED_OUTDIR/g"`
