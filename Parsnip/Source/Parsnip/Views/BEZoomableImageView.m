...
@implementation BEZoomableImageView
{
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        _doubleTapZoomMode = BEZoomModeActualSize;
...
- (void)onDoubleTap:(UITapGestureRecognizer *)recognizer
{
    ...
    if (_doubleTapZoomMode != BEZoomModeStep) {
        ...
        if (ABS(offset.x) < 0.00001 && ABS(offset.y) < 0.00001 && self.imageScale == scaleToFit) {
            if (_doubleTapZoomMode == BEZoomModeActualSize) {
                scale = 1.0f;
